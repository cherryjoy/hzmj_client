...
static public class NGUIText
{
	...
	static public void Align(BetterList<Vector3> verts, int indexOffset, TextAlignment alignment, int pos, int lineWidth, int fontSize)
	{
...
#if DYNAMIC_FONT
    static public Vector2 CalculatePrintedSize(string text, UILabel lbl)
    {
        Font font = lbl.font.TrueTypeFont;
        int size = lbl.FontSize;
        FontStyle style = lbl.FontStyle;
        bool encoding = lbl.supportEncoding;

        return CalculatePrintedSize(text, font, size, style, encoding);
    }
    ...
    /// <summary>
    /// return the index of char in string,that length fit the given length
    /// </summary>

    static public int GetCharIndexFitLength(string text, float lengthLimit,UILabel lbl)
    {
        int i = 0;
    ...
    static public float GetStringPrintLength(string text, float lengthLimit, UILabel lbl)
    {
        int i = 0;
        ...
        Vector2 printSize = CalculatePrintedSize(text, font, size, style, encoding);
	...
	static public void Print(string text, Font font, int size, FontStyle style, Color32 color,
		...
		BetterList<Color> cols, Vector3 scale, Color topColor, Color bottomColor, int spaceY, int spaceX, 
        int width, int height)
	{
		...
		font.RequestCharactersInTexture("a", size, style);
		font.GetCharacterInfo('a', out mTempChar, size, style);
		baseline = size + mTempChar.vert.yMax + 2 ;
		...
		for (int i = 0; i < textLength; ++i)
		{
			...
			if (c == '\n')
			{
				...
				if (alignment != TextAlignment.Left)
				{
					Align(verts, indexOffset, alignment, x, lineWidth, width);
			...
			v0.x = scale.x * (x + mTempChar.vert.xMin);
			v0.y = -scale.y * (y - mTempChar.vert.yMax + baseline);

			v1.x = v0.x + scale.x * mTempChar.vert.width;
			v1.y = v0.y - scale.y * mTempChar.vert.height;
			...
			x += (int)mTempChar.width + spaceX;

			Color c0 = color * topColor;
			Color c1 = color * bottomColor;
    ...
    /// <summary>
    /// Print the 3 color gradient text into the buffers.
    /// </summary>
    ...
    static public void Print(string text, Font font, int size, FontStyle style, Color32 color,
        ...
        BetterList<Vector3> verts,
        BetterList<Vector2> uvs,
        BetterList<Color> cols, Vector3 scale, Color topColor, Color middleColor, Color bottomColor, int spaceY, int spaceX, int width, int height)
    {
        if (font == null || string.IsNullOrEmpty(text)) return;
        ...
        for (int i = 0; i < textLength; ++i)
        {
            ...
            Color c0 = color * topColor;
            Color c1 = color * middleColor;
            Color c2 = color * bottomColor;


            if (mTempChar.flipped)
            {
                uvs.Add(new Vector2((u0.x+u1.x)*0.5f, u1.y));
                uvs.Add(new Vector2((u0.x + u1.x) * 0.5f, u0.y));
                uvs.Add(new Vector2(u1.x, u0.y));
                uvs.Add(new Vector2(u1.x, u1.y));
            ...
            {
                uvs.Add(new Vector2(u1.x,(u0.y+u1.y)*0.5f ));
                uvs.Add(new Vector2(u0.x, (u0.y+u1.y)*0.5f));
                uvs.Add(new Vector2(u0.x,u1.y ));
                uvs.Add(new Vector2(u1.x,u1.y ));
            ...
            verts.Add(new Vector3(v1.x, (v0.y + v1.y) * 0.5f));
            verts.Add(new Vector3(v0.x, (v0.y + v1.y) * 0.5f));
            verts.Add(new Vector3(v0.x, v1.y));
            verts.Add(new Vector3(v1.x, v1.y));
