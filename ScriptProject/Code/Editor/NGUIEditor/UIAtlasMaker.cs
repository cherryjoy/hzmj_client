...
public class UIAtlasMaker : EditorWindow
{
	class SpriteEntry
	{
        ...
        public bool UpdateOuterOffset = false;
	}
	...
	static UIAtlas.Sprite AddSprite (List<UIAtlas.Sprite> sprites, SpriteEntry se)
	{
		...
		if (sprite != null)
		{
            ...
            sprite.oriOuter = se.rect;
            sprite.outer = NGUIMath.RectAdd(se.rect,sprite.outerOffset);
            Debug.Log(sprite.name + sprite.outerOffset);
			sprite.inner = se.rect;

            if (se.UpdateOuterOffset)
            {
                sprite.outerOffset = new Rect(0, 0, 0, 0);
                se.UpdateOuterOffset = false;
            }
		...
		foreach (Texture tex in textures)
		{
			...
			if (newWidth > 0 && newHeight > 0)
			{
				...
				else
				{
					...
					sprite.tex = new Texture2D(newWidth, newHeight, TextureFormat.RGBA32, false);
		...
		if (atlasTex != null)
		{
			...
			foreach (UIAtlas.Sprite asp in list)
			{
				...
				if (!found)
				{
                    ...
                    if (asp.oriOuter.xMin == 0 && asp.oriOuter.yMin == 0 && asp.oriOuter.width == 1 && asp.oriOuter.height == 1)
                    {
                        asp.oriOuter = asp.outer;
                    }
                    Rect rect = asp.oriOuter;
	...
	void UpdateAtlas (List<Texture> textures, bool keepSprites)
	{
		...
		if (sprites.Count > 0)
		{
            foreach (SpriteEntry s in sprites) {
                s.UpdateOuterOffset = true;    
            }
